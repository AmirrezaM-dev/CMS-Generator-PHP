    ...
    if(isset($connected) && $connected == 1 && isset($show_tables) && isset($needed_tables) && count(array_intersect($show_tables, $needed_tables)) >= count($needed_tables)){
        if(isset($_SESSION['username'])){
            ...
            if($user_stats == 1 || isset($op_admin) && $op_admin){
				if(isset($_POST[$last_name['permissions']['tables_permission'].'_name']) && isset($_POST['primaryKey']) && isset($_POST['custom_filter'])){
					$custom_filter=json_decode($_POST['custom_filter']);
					$table_name = $_POST[$last_name['permissions']['tables_permission'].'_name'];
						...
						if(isset($op_admin) && $op_admin == 1 || $table_get['visible'] == 1){
							if(checkPermission(1, $table_id, "read", $table_get['act'], "") == 1){
								...
								array_push($columns, [
									...
									'dt' => 4,
									'formatter' => function ($d, $row){
										$status=$GLOBALS['connection']->query("SELECT * FROM ".$GLOBALS['sub_name']."table_permissions WHERE id='".$d."'")->fetch()['act'];
										if($GLOBALS['user_language'] == 'en'){
											$status=($status==1 ? "Enabled":"Disabled");
										}else{
											$status=($status==1 ? "فعال شده":"غیرفعال شده");
										}
										return $status.' <a href="javascript:void(0)" onclick="action_permission_'.$GLOBALS['last_name']['permissions']['tables_permission'].'('."'edit'".',this,'.$d.',event);$(this).children().removeClass('."'tim-icons icon-pencil'".').addClass('."'far fa-spin fa-spinner'".');" class="btn btn-link btn-info btn-icon btn-sm"><i class="tim-icons icon-pencil"></i></a>';
									}
								]);
								...
								$txt = json_encode(SSP::complex_permission_check($_POST, $sql_details, $table_name, $primaryKey, $columns,null,null,$custom_filter));
